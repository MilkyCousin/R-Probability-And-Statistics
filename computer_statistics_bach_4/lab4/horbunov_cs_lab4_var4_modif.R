...
h <- function(t, m.1, m.2, s.1, s.2, p)
{
  ...
  g <- p/(2*sqrt(2*pi)) * exp(-0.5 * (t - m.1)^2/s.1^2) * ((t - m.1)^2 * s.1^(-5) - s.1^(-3))
...
hist(sqrt(1000)*(UU[UU<.6] - true.theta^2 ), main = "Histogram of sqrt(n)*(theta_n - theta^2)", probability = T, breaks = 50); grid()
